...
Custom tags need to be transformed to JavaScript before the browser can execute them. You can do this by setting a `type="riot"` attribute for your script tags. For example:
...
<script src="https://cdn.jsdelivr.net/npm/riot@{{ site.version }}/riot+compiler.min.js"></script>
...
**The the external file can contain only one tag definition**.
...
The Compilation phase is asynchronous and it will not block your application rendering. However you should use the browser compilation only for prototyping or for quick experiments, bundling your riot tags via [webpack](https://github.com/riot/webpack-loader) or [rollup](https://github.com/riot/rollup-plugin-riot) is highly recommended (see [Pre-Compilation](#pre-compilation)).
